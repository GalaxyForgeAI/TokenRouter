...
TokenRouter exposes a native Gemini `v1beta` compatibility surface. The official Gemini CLI can connect directly to TokenRouter, which then routes the request to an OpenAI Codex Subscription account. CCswitch and other local protocol proxies are not required.

## Prerequisites

- A healthy OpenAI Codex Subscription account is configured in TokenRouter.
- A GPT model such as `gpt-5.5` is enabled and routed to that provider.
- The TokenRouter project key allows that model.
...
```bash
export TOKENHUB_GEMINI_KEY='your TokenRouter project key'
...
For a local TokenRouter instance:
...
Gemini CLI sends `GEMINI_API_KEY` in `x-goog-api-key`; use a TokenRouter project key, not an OpenAI OAuth access token.
...
```dotenv
GEMINI_API_KEY=your TokenRouter project key
GOOGLE_GEMINI_BASE_URL=https://tokenhub.example.com
GEMINI_MODEL=gpt-5.5
```
