...
| [Codex 接入 TokenRouter：Profile 快速配置](codex-tokenhub-profile-quick-start.md) | 仅需使用独立 Profile 的 Codex 用户 | 快速完成配置文件创建、Key 设置、启动验证和恢复 |
| [Codex 接入 TokenRouter：四种配置方式与恢复指南](codex-tokenhub-configuration.md) | 需要比较不同接入方式的 Codex 用户和项目开发者 | 使用 Profile、进程级临时配置、CLI 全局配置或桌面端配置 TokenRouter |
