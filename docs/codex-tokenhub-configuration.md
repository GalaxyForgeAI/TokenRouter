# Connect Codex to TokenRouter: Four Configuration Methods and Recovery

Language: English | [简体中文](zh-CN/codex-tokenhub-configuration.md) | [日本語](ja/codex-tokenhub-configuration.md)

> This guide connects local Codex CLI, the Codex desktop app, and the IDE extension to TokenRouter. It covers an isolated profile, process-local overrides, global CLI configuration, and desktop configuration.
>
...
| Global CLI configuration | Current user's local Codex sessions | Yes | CLI uses TokenRouter by default | Restore `config.toml` |
...
The TokenRouter console login token is not a project API key. Never commit API keys or place them in shell history. Prefer `env_key`; `experimental_bearer_token` is available only for controlled development use and stores the key in plaintext.
...
| --- | --- | --- |
| TokenRouter Base URL | Deployment information | Ends with `/v1` |
| TokenRouter project API key | **Key Management** in TokenRouter | Project API key, not login token |
...
```bash
export TOKENHUB_BASE_URL="enter the actual TokenRouter Base URL"
...
```bash
export TOKENHUB_BASE_URL="enter the actual TokenRouter Base URL"
read -r -s -p "TokenRouter project API key: " TOKENHUB_API_KEY
export TOKENHUB_API_KEY
echo
```
...
```powershell
$env:TOKENHUB_BASE_URL = Read-Host "TokenRouter Base URL (must end with /v1)"
$tokenHubSecureKey = Read-Host "TokenRouter project API key" -AsSecureString
...
name = "TokenRouter"
base_url = "enter the actual TokenRouter Base URL"
...
Run `/status` and confirm the model, TokenRouter provider, and corresponding successful TokenRouter request log.
...
Run `/status` and confirm the request in TokenRouter logs.

To recover, restore the backup or return `model_provider` and `model` to their previous values and remove the TokenRouter provider table. Restart Codex completely.
...
```dotenv
TOKENHUB_API_KEY=enter the actual TokenRouter project API key
```
...
Restart Codex completely, create a local task, confirm the model, and verify the request in TokenRouter logs. Local `config.toml` does not control the default model for Codex cloud tasks.
...
| HTTP 401 / `invalid_api_key` | Missing, malformed, or unrecognized project key | Use a TokenRouter project API key, not a console login token |
